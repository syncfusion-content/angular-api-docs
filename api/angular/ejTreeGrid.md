...
Enables or disables the ability to filter the data on all the columns. Enable this property to display a row with editor controls corresponding to each column. You can restrict filtering on a particular column by disabling this property directly on that column instance itself.            
...
Enables or disables the ability to sort the rows based on multiple columns or fields by clicking each column header. Rows will be sorted recursively by clicking the column headers.
...
Enables or disables the ability to sort the rows based on a single field or column by clicking that column header. When enabled, rows can be sorted by a single field or column.
...
Enables or disables the pagination of rows in tree grid.
...
Enables the blanks option in the ejDropDownList mapped in tree grid column.
...
Enables or disables the ability to sort the rows based on this column or field.
...
Enables or disables the cell selection.
...
To customize the ej controls defined in tree grid column with their native property.
...
To bind the dropdown data for tree grid column mapped with the ejDropDownList.
...
Specifies the template string of the script element to enable the column template for a column.
...
Specifies whether the column is a template column or not.
...
Gets or sets an object to define a command column in tree grid.
...
Gets or sets an object to customize command button with the available ejButton properties.
...
Gets or sets a value that defines the command column buttons to be displayed.
...
A particular column can be included or excluded from the column visibility list in column menu.
...
Sets the clip mode for tree grid cell as ellipsis or clipped content (both header and content.)
...
Gets or sets the priority value of the column. It is used to show or hide the tree grid columns in responsive mode.
...
Specifies the column fields to be displayed in the dialog while inserting a column using column menu.
...
Enables or disables the ability to freeze or unfreeze the columns.
...
Options for displaying and customizing the context menu items.


### contextMenuSettings.contextMenuItems `array`
{:#members:contextmenusettings-contextmenuitems}

Option for adding items to the context menu.
...
Shows or hides the context menu.
...
Specifies the CSS class for tree grid to achieve custom theme.
...
Specifies hierarchical or self-referential data to populate the tree grid.
...
Specifies whether to wrap the header text or when it overflows or exceeds the header width. 
...
Options to display and customize the tooltip. This tooltip will show the preview of the row that is being dragged.


### dragTooltip.showTooltip `boolean`
{:#members:dragtooltip-showtooltip}

Specifies whether to show the tooltip while dragging a row.
...
Specifies the mouse action by single or double click to begin the editing.
...
specifies the edit mode in tree grid , "cellEditing" is for cell type editing and "rowEditing" is for entire row.
...
Specifies whether to load all the rows in collapsed state when the tree grid is rendered for the first time.
...
Specifies whether to resize tree grid whenever the window size changes.
...
Defines the common width for all the columns in tree grid.
...
Options to filter and customize the filter actions.            

### filterSettings.filterBarMode `string`
{:#members:filtersettings-filterbarmode}

Specifies the start mode of column filter.         
...
Specifies the column collection to filter the tree grid content on initial loading. 
...
Specifies the filter condition of column filter. See <a href="global.html#enum:filteroperator">operator</a>.
...
Enables or disables internal parsing of a row. The row will be displayed using the defined template without any internal event bindings when this property is disabled.
...
Enables or disables the responsiveness of tree grid.
...
Specifies the options to customize the pager.


### pageSettings.pageCount `number`
{:#members:pagesettings-pagecount}

Using this property, you can specify the number of pages, according to this count tree grid height will be updated.
...
Get the value of records that bound to tree grid. The totalRecordsCount value is calculated based on the datasource bound to tree grid.
...
Specifies the mode of record count in a page that is whether it should count all the records or the root to count zero level parent records.  
...
Specifies the mode of printing the control that is whether it should print the all records or the current page content.           
...
Specifies the ej.Query property to select data from the dataSource. This property is applicable only when the dataSource is ej.DataManager.
...
Specifies the height of a single row in tree grid. Also, set same height in the CSS style with the class name e-rowcell.
...
Specifies the selected cell information on rendering the tree grid.

### selectedCellIndexes.rowIndex `number`
{:#members:selectedcellindexes-rowIndex}


Specifies the row index of the cell to be selected in tree grid control.    
...
Specifies the type of selection whether to select a row or cell.          
...
Toggles the visibility of the checkbox in column header using all the check boxes to be selected or unselected.
...
Enables or disables the options for inserting , deleting, and renaming  the columns.
...
Controls the visibility of the menu button, which is displayed on the column header. Clicking this button will show a popup menu. When you choose `Columns` item from this popup, a list box with column names will be shown, from which you can select or deselect a column name to control the visibility of the respective columns.
...
Specifies the visibility of the expander column used to expand or collapse the details view.
...
Specifies the title for summary row collection in tree grid.
...
Specifies the summary column used to perform the summary calculation.
...
Specifies whether to show tooltip when mouse is hovered over the cell.
...
Options for setting width and height for tree grid.
...
Specifies the field to be sorted in tree grid.
...
Specifies the sort direction in tree grid.
...
Specifies the list of toolbar items to be rendered in tree grid toolbar.
...
Allows the user to insert the custom icons in toolbar using script templates. Using this property, we can bind HTML elements and other EJ controls to tree grid toolbar.
...
Allows the user to display custom tooltip text for tree grid custom toolbar items.
...
Set the allowAdding to true to add a new row in tree grid.
...
To clear all the selection in tree grid.
...
Freezes all the columns preceding to the column specified by the field name.
...
Freezes or unfreezes the specified column.
...
To save the edited cell in tree grid.
...
To sort the data based on the particular fields.
...
Triggers before every success event of tree grid action.
...
Triggers for every tree grid action success event.
...
Triggers before the printing initiated in tree grid.
...
Triggers while entering the edit mode in the tree grid cell.
...
Triggers after the tree grid collapsed.
...
Triggers while collapsing the tree grid record.
...
Triggers when you start to drag a column.
...
Triggers while dragging a column. 
...
Triggers after a column resized.
...
Triggers when you start to resize a column.
...
Triggers when a column has been resized.
...
Triggers when the context menu is rendered in tree grid control.
<table class="params">
<thead>
<tr>
<th>Name</th>
...
Triggers after the tree grid is rendered.
...
Triggers while rendering template details in tree grid.
...
Triggers when template pop-up details are shown.
...
Triggers when details template pop-up is hidden.
...
Triggers after saving the modified cellValue in tree grid.
...
Triggers after expanding the record.
...
Triggers while expanding the tree grid record.
...
Triggers while loading the tree grid.
...
Triggers while rendering each cell in the tree grid.
...
Triggers while rendering each row.
...
Triggers while dragging a row in the tree grid control.
...
Triggers when row dragging starts in the tree grid control.
...
Triggers while dropping a row in the tree grid control.
...
Triggers after selecting a cell.
...
Triggers after the row is selected.    
...
Triggers before selecting the row.
...
Triggers while clicking a row, even when the allowSelection property is disabled.          
...
Triggers during record double click action, even when the allowSelection property is disabled.                                       
...
Triggers when you click the toolbar item in tree grid. 
